...
import {BASE_API_URL as BASE, METHODS, TOKEN_HEADER_NAME, TOKEN_COOKIE_ID} from './config';
...
class Token {
    ...
    set(token: string, expires_in: number =3600): void {
        this.cookies.set(TOKEN_COOKIE_ID, token, {path: '/', expires: new Date(Date.now() + expires_in*1000)});
    }
